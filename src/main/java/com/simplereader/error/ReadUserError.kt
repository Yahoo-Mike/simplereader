...
fun ReadError.toUserError(): UserError = when (this) {
    is ReadError.Access ->
        when (val cause = this.cause) {
            ...
            else -> UserError(R.string.publication_error_unexpected, cause = this)
        }
...
fun ContentResolverError.toUserError(): UserError = when (this) {
    ...
    is ContentResolverError.NotAvailable -> UserError(R.string.publication_error_unexpected, cause = this)
}
